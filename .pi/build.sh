...
# docker buildx build --tag polynetworkcosmos --file .pi/Dockerfile --platform linux/arm64 --cache-from polynetworkcosmos:cache --cache-to polynetworkcosmos:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar polynetworkcosmos
...
sudo bash -c "echo polynetworkcosmos > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/polynetworkcosmos.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/polynetworkcosmos.img
